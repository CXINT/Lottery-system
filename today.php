...
<body bgcolor="#626262">
    ...
    <div class="war" style="padding-top:10px; overflow:hidden;">
        ...
        <div class="right">
				...
				$time = time();
				$ti= '20'.date("y-m-d",$time);
                $Page_size=10; 
                $wfstatus=$_GET['wfstatus'];
				$sql_count = "select * from wforderlist where  (date(wfdate)='$ti' or date(wfeditdate)='$ti') and wfproduct like '%$product%'"; 
				if($wfstatus==""){
					$sql_count = "select * from wforderlist where  (date(wfdate)='$ti' or date(wfeditdate)='$ti') and wfproduct like '%$product%' "; 	
				}
				if($wfstatus!=""){
						$sql_count= $sql_count." and wfstatus='$wfstatus'";
				}
                ...
                $sql="select * from wforderlist  where  (date(wfdate)='$ti' or date(wfeditdate)='$ti') "; 
				if($wfstatus==""){
					$sql = "select * from wforderlist where  (date(wfdate)='$ti' or date(wfeditdate)='$ti') and wfproduct like '%$product%' "; 	
				}
				if($wfstatus!=""){
						$sql= $sql." and wfstatus='$wfstatus' and wfproduct like '%$product%'";
				}
                ...
                if($page!=1){ 
                $key.="<a href=\"".$_SERVER['PHP_SELF']."?wfstatus=$wfstatus&page=1\">第一页</a> "; //第一页 
                $key.="<a href=\"".$_SERVER['PHP_SELF']."?wfstatus=$wfstatus&page=".($page-1)."\">上一页</a>"; //上一页 
                }else { 
                $key.="第一页 ";//第一页 
                $key.="上一页"; //上一页 
                } 
                ...
                if($page!=$pages){ 
                $key.=" <a href=\"".$_SERVER['PHP_SELF']."?wfstatus=$wfstatus&page=".($page+1)."\">下一页</a> ";//下一页 
                $key.="<a href=\"".$_SERVER['PHP_SELF']."?wfstatus=$wfstatus&page={$pages}\">最后一页</a>"; //最后一页 
                }else { 
                $key.="下一页 ";//下一页 
                $key.="最后一页"; //最后一页 
                } 
                ...
                <td colspan="14" bgcolor="#E0EEE0">
                	...
                	<?php	
							//$he= mysql_query("select sum(wfprice) as count from($sql) bieming");//派生表别名
							//$h= "select sum(wfprice) as count from($sql) bieming";//派生表别名
							//echo $h;
                	...
                	<div align="center"><?php echo $key?>
		                 <form method="get" name="form11111" style="float:left; margin-top:5px;">
							...
							<input type="hidden" name="wfstatus" value="<?php echo "$wfstatus";?>"/>
